  ...
  for (let i = 0; i < orders.length; i++) {
    ...
    if (orders[i].status !== "Pending") {
      for (let n = 0; n < orders[i].products.length; n++) {
        products.push(
          orders[i].products[n].quantity +
            "x " +
            orders[i].products[n].name +
            " " +
      ...
      order_cards.push(
        <OrderCard
          ...
          date={orders[i].timestamp.slice(0, 10)}
          billedTo={
            orders[i].billing_address.first_name +
            " " +
            orders[i].billing_address.last_name +
            ". " +
            orders[i].billing_address.address +
            ". " +
            orders[i].billing_address.city +
            " (" +
            orders[i].billing_address.state +
            "), " +
            orders[i].billing_address.country +
            ". " +
            orders[i].billing_address.zip_code
          }
          shippedTo={
            orders[i].shipping_address.first_name +
            " " +
            orders[i].shipping_address.last_name +
            ". " +
            orders[i].shipping_address.address +
            ". " +
            orders[i].shipping_address.city +
            " (" +
            orders[i].shipping_address.state +
            "), " +
            orders[i].shipping_address.country +
            ". " +
