...
import {
  getProductsAction,
  changePageAction
} from "../../actions/product-management/getProductActions";
...
const ViewProducts = props => {
  ...
  const total_count = useSelector(
    state => state.viewProductsState.total_products
  );
  const opened_page = useSelector(state => state.viewProductsState.opened_page);
  const deleted_product = useSelector(
    state => state.viewProductsState.deleted_product
  );
  ...
  useEffect(() => {
    ...
    dispatch(getProductsAction(opened_page, page_size, searchParameters));
  }, [opened_page, deleted_product, dispatch, props.location.search]);
  ...
  let pages = [];
  for (let page = 0; page < total_count / page_size; page++) {
    pages.push(
      <div
        key={page}
          ...
          <div className="row col-10 offset-1">
            <div className="col-12 col-md-6 ProductsHeader">
              <div className="row">
                ...
                <Link to={"/admin/products/add"} className="LinkToAdd">
                  {" "}
                  <div className="AddProductIcon"> + </div>{" "}
                </Link>
              </div>
            </div>
            <div className="col-12 col-md-6 SearchBarHeader">
              <SearchBar
                ...
                search_redirect_base="/admin/products"
              />
            </div>
          </div>
          <div className="row col-10 offset-1">
            <div className="row justify-content-center">{productsToManage}</div>
          </div>
          <div className="row">
